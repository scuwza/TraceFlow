package com.wza.TraceFlow.support.mq.rabbit;
...
@Slf4j
@Service
@ConditionalOnProperty(name = "TraceFlow.mq.pipeline", havingValue = MessageQueuePipeline.RABBIT_MQ)
public class RabbitSendMqServiceImpl implements SendMqService {

    @Autowired
    private RabbitTemplate rabbitTemplate;

    @Value("${TraceFlow.rabbitmq.topic.name}")
    private String confTopic;

    @Value("${TraceFlow.rabbitmq.exchange.name}")
